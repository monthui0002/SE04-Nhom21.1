package com.jkgh.jvm.runtime {
	...
	public class MutableBigInteger {
		...
		public static function forNumber(value:Number):MutableBigInteger {
			if (isNaN(value))
				throw new Error("Illegal NaN parameter: " + value);
			if (!isFinite(value))
				throw new Error("Illegal infinite parameter: " + value);
			if (value == 0)
				return ZERO;
			if (value < 0)
				throw new Error("Illegal negative parameter: " + value);
		...
		public static function forString(digits:String, radix:int = 10):MutableBigInteger {
			...
			if (!radixConstraints.validate(digits))
				throw new Error("Illegal digit(s) for radix " + radix + ": " + digits);
